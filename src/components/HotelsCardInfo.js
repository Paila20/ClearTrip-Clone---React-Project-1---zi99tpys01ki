...
export default function HotelsCardInfo() {
  ...
  const { all, setall, tokenAvailability,checklogin } = useAuthContext();
...
function scrolleffect() {
  ...
  function navigatedetailspage(){
      ...
      navigate(`/hotels/results/hotelInfo/Info?hotel_id=${dataa._id}&rooms=${rooms}&adults=${adults}&childrens=${childrens}&date=${dayOfWeek}
       &roomno=${sidebardata.roomNumber}
        `)
                ...
                <div className='cardupperdiv flex g20'>
                  {dataa.rooms.map((item, index) => (
                    <div key={index} className='roomcardhotel flex flexc'>
                      ...
                      <button className='hotelroombookbutton' onClick={() => { fulldetailpagedirectionchanger();
                         setsidebardata(item)
                          }}>Book</button>
                    </div>
                  ))}
