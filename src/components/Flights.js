...
import { objdropdowncity ,objcolor,objfares,months,daysOfWeek } from './Constant';
              ...
              <div className='inOutCombine b1 flexa'>
                <div className='flexa'>
                  <div className='ii1 flexa'>
                    ...
                    <input type='text' value={flightIn} onClick={() => { setflightInOutPop({}); setflightInOutPop({ ["in"]: !flightInOutPop["in"] }) }} onChange={(e) => { setflightIn(e.target.value) }} placeholder='Where from?' />
                    {flightInOutPop["in"] == true && <div className='flightInData flexa flexc'>

                      {objdropdowncity.map((item) => (
                        <div className='slidee flexja' onClick={() => { setflightIn(`${item.name} - ${item.fname}`); setflightIndata(item); setflightInOutPop({}) }}>
                          <p>{item.name}</p>
                ...
                <div className='i2 flexa'>
                  <div className='ii2 flexa'>
                    ...
                    <input type='text' value={flightOut} onClick={() => { setflightInOutPop({}); setflightInOutPop({ ["out"]: !flightInOutPop["out"] }) }} onChange={(e) => { setflightOut(e.target.value) }} placeholder='Where to?' />
                    {flightInOutPop["out"] == true && <div className='flightInData flexa flexc'>
                      {objdropdowncity.map((item) => (
                        <div className='slidee flexja' onClick={() => { setflightOut(`${item.name} - ${item.fname}`); setflightOutdata(item); setflightInOutPop({}) }}>
                          <p>{item.name}</p>
                          <h4>{item.fname} {item.lname}</h4>
                        </div>
                      ))}
